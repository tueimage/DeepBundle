...
## LOAD DATA
def LoadData(save, path_save, path_load):
    id_all = random.sample(range(params.n_tot),params.train+params.test+params.val+params.pseudo) # Random
...
if params.load:
    ...
    Xtrain, ytrain, Xtest, ytest, Xval, yval, Xpseudo, ypseudo, min_dist_l, L = LoadData(params.save, params.saveDirectory, params.loadDirectory)
...
params1 = dict(
    ...
    learning_rate = 8e-6,              # Learning rate for training
    decay_rate = 0.95,                 # Decay rate
    decay_steps = int(0.1*30*Xtrain.shape[0] / 64),         # Decays 10x with decay_rate
...
if params.FPMining:
    ...
    params3 = dict(
        ...
        imbalance = list(ytrain_added).count(0)/list(ytrain_added).count(1) # Likely imbalanced
    ) 
