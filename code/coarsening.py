"""
This code is adapted from ChebNet which looks at a spectral convolution technique introduced in 
the paper by M. Defferrard, X. Bresson, and P. Vandergheynst, on 
“Convolutional Neural Networks on Graphs with Fast Localized Spectral Filtering”

https://github.com/dsgiitr/graph_nets/blob/master/ChebNet/coarsening.py
"""
...
def laplacian(W, normalized=True):
    ...
    d = W.sum(axis=0) # Degree matrix.

    if not normalized:
        D = scipy.sparse.diags(d.A.squeeze(), 0)
        L = D - W # Non-normalized Laplacian
    else:
        d += np.spacing(np.array(0, W.dtype))
        d = 1 / np.sqrt(d)
        D = scipy.sparse.diags(d.A.squeeze(), 0)
        I = scipy.sparse.identity(d.size, dtype=W.dtype)
        L = I - D * W * D # Normalized laplacian
        ...
        if not marked[tid]:
            ...
            for jj in range(rowlength[tid]):
                ...
                if marked[nid]:
                    ...
                    if 2 == 1: # First approach
                        tval = vv[rs+jj] * (1.0/weights[tid] + 1.0/weights[nid])

                    if 1 == 1: # Second approach
                        Wij = vv[rs+jj]
                        Wii = vv[rowstart[tid]]
                        Wjj = vv[rowstart[nid]]
                        di = weights[tid]
